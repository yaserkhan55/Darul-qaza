...
export default function AdminCaseReview({ caseData, onClose, onUpdate }) {
    ...
    // File number is now generated by the backend service on approval
    // to ensure uniqueness and per-user consistency.
    ...
    const handleApprove = async () => {
        ...
        try {
            const payload = {
                adminMessage: unifiedMessage || `Your case has been approved.`,
                decisionComment: (unifiedMessage || "Case approved").trim()
            };

            const updatedCase = await adminApi.approveDarkhast(caseData._id, payload);
            alert(`Case approved successfully! ${updatedCase.fileNumber ? 'File Number: ' + updatedCase.fileNumber : ''}`);
            onUpdate && onUpdate();
            onClose && onClose();
        } catch (err) {
