export default function FaislaView({ caseData }) {
    ...
    if (!faisla) return (
        <div className="bg-white rounded-3xl p-12 text-center border border-emerald-50 shadow-inner">
            ...
            <h3 className="text-xl font-bold text-islamicGreen tracking-tight">Awaiting Final Decision</h3>
            <p className="text-gray-500 mt-2 font-medium">The Qazi is preparing the final Order Sheet (Faisla). Please wait.</p>
        </div>
    );
        ...
        <div className="max-w-4xl mx-auto space-y-10 pb-20">
            ...
            <div className="bg-white rounded-3xl p-8 sm:p-12 shadow-2xl relative border border-emerald-50 overflow-hidden">
                <div className="bg-emerald-50/30 -mx-12 -mt-12 p-12 text-center border-b border-emerald-100 mb-12">
                    <h1 className="text-4xl font-black text-islamicGreen tracking-tight uppercase">Order Sheet</h1>
                    <p className="text-[10px] font-black text-gray-400 mt-4 uppercase tracking-[0.4em]">Official Judicial Record • Dar-ul-Qaza Lahore</p>
                </div>
                ...
                <div className="space-y-12 relative z-10">
                    <div className="grid grid-cols-2 gap-8 border-b border-emerald-50 pb-8">
                        <div className="space-y-4">
                            <div>
                                <span className="text-[10px] font-black uppercase text-gray-300 block mb-1">Case ID</span>
                                <span className="text-2xl font-black text-gray-900 font-mono tracking-tighter italic">#{caseId?.slice(-8).toUpperCase()}</span>
                            </div>
                            <div>
                                <span className="text-[10px] font-black uppercase text-gray-300 block mb-1">Matter</span>
                                <span className="text-xl font-bold text-islamicGreen">{type}</span>
                            </div>
                        </div>
                        <div className="text-right space-y-4">
                            <div>
                                <span className="text-[10px] font-black uppercase text-gray-300 block mb-1">Date of Faisla</span>
                                <span className="text-2xl font-black text-gray-900">{new Date(faisla.decisionDate).toLocaleDateString()}</span>
                            </div>
                            <div>
                                <span className="text-[10px] font-black uppercase text-gray-300 block mb-1">Registry Ref</span>
                                <span className="text-xl font-bold text-gray-900 uppercase tabular-nums">{faisla.courtSealRef || 'DQ/LHR/2024-X'}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="pt-12 flex flex-col items-center">
                        <div className="text-center">
                            <div className="w-64 border-t-2 border-emerald-50 pt-4 mb-8">
                                <p className="text-[10px] font-black uppercase tracking-widest text-gray-300 mb-1">Official Qazi Signature</p>
                            ...
                            <div className="inline-block p-4 border border-islamicGreen/20 rounded-full opacity-30 transform scale-110">
                                <span className="text-[10px] font-black text-islamicGreen uppercase tracking-widest">Seal of Dar-ul-Qaza</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-islamicGreen rounded-3xl p-8 sm:p-12 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-8 text-white relative overflow-hidden group">
                ...
                <div className="space-y-2 text-center md:text-left relative z-10">
                    <h4 className="text-3xl font-black tracking-tight">Download Certificate</h4>
                    <p className="text-emerald-100 text-sm max-w-md italic font-medium">
                        Your final judgment has been formalized. You may now download your Shariah-compliant divorce certificate.
                    </p>
                </div>
                <button
